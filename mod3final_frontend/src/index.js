...
//**********************GAME INIT***********************
	...
	function checkCollision() {
		...
		if (
			state.projectile_position.x <= 110 &&
			state.projectile_position.x >= 60 &&
			jumping == false
		) {
			drawEnd()
			// console.log("timer is:", timer)
		...
		if (collision === true) {
			...
			// render the
			showForm()
			//render the sgin in form, ask for user name
			// listen to the submit
			//submit fire create user fist then create game
...
formField.addEventListener("submit", ev => {
	...
	function renderPlayerName(player) {
		...
		adapter.createGame(gameData)
	}
	adapter.createPlayer(playerName).then(renderPlayerName)
	// debugger
	// app.startgame
